...
function generateOHLC(basePrice, dateStr) {
  const v     = basePrice * 0.012;
  const open  = basePrice + (Math.random() - 0.5) * v;
  const close = basePrice + (Math.random() - 0.5) * v;
...
function buildOHLCData(transactions, currentPrice) {
  ...
  if (transactions && transactions.length > 0) {
    const sorted = [...transactions].sort((a, b) => new Date(a.tradeDate) - new Date(b.tradeDate));
    const txByDate = {};
    for (const tx of sorted) {
      const txDate = toISODate(tx.tradeDate);
      txByDate[txDate] = tx;
      const isBuy = (tx.status || '').toUpperCase() === 'BUY';
      markers.push({ time: txDate, position: isBuy ? 'belowBar' : 'aboveBar', color: isBuy ? '#10b981' : '#ef4444', shape: isBuy ? 'arrowUp' : 'arrowDown', text: isBuy ? 'BUY' : 'SELL' });
    }

    const firstDate = toISODate(sorted[0].tradeDate);
    const ohlc = [], volume = [];
    let prevClose = sorted[0].avgPrice ?? currentPrice;
    for (let i = 0; i <= getDaysBetween(firstDate, today); i++) {
      const date   = addDays(firstDate, i);
      if (date > today) break;
      const tx     = txByDate[date];
      const candle = generateOHLC(tx ? (tx.avgPrice ?? prevClose) : prevClose, date);
      prevClose    = candle.close;
      ohlc.push(candle);
      const vol = Math.round(50000 + Math.random() * 150000);
      volume.push({ time: date, value: vol, color: candle.close >= candle.open ? '#10b981' : '#ef4444' });
    }
  ...
  for (let i = 180; i >= 0; i--) {
    ...
    price = Math.min(Math.max(candle.close, currentPrice - bound), currentPrice + bound);
...
export default function StockChart({ symbol, transactions, currentPrice }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      height: 400,
      layout: { background: { color: '#151821' }, textColor: '#9ca3af' },
    ...
    const { ohlc, volume, markers } = buildOHLCData(transactions || [], currentPrice ?? 0);

    if (ohlc.length > 0) {
      candleSeries.setData(ohlc);
      volumeSeries.setData(volume);
      candleSeries.setMarkers(markers);
      candleSeries.createPriceLine({ price: currentPrice ?? 0, color: '#60a5fa', lineWidth: 2, lineStyle: 2, title: 'Current' });

      const to   = new Date();
      const from = new Date();
      from.setDate(from.getDate() - 7);
      chart.timeScale().setVisibleRange({ from: Math.floor(from.getTime() / 1000), to: Math.floor(to.getTime() / 1000) });
    }
  ...
  function setPeriod(label) {
    ...
    const to     = new Date();
    const from   = new Date();
    if (period?.days) from.setDate(from.getDate() - period.days);
    else from.setMonth(from.getMonth() - (period?.months ?? 6));
    chart.timeScale().setVisibleRange({ from: Math.floor(from.getTime() / 1000), to: Math.floor(to.getTime() / 1000) });
  }
  ...
  return (
    <div className="stock-chart-wrapper">
      <div className="chart-toolbar">
        {PERIODS.map(p => (
          <button key={p.label} type="button" className={`chart-period-btn ${activePeriod === p.label ? 'active' : ''}`} onClick={() => setPeriod(p.label)}>
            {p.label}
          </button>
        ))}
      </div>
